...
function app_navs($type = 'home', $multiid = 0, $section = 0) {
	...
	$sql = "SELECT id,name, description, url, icon, css, position, module,look FROM " . tablename('site_nav') . " WHERE position = '{$pos[$type]}' AND status = 1 AND uniacid = '{$_W['uniacid']}' AND multiid = '{$multiid}' ORDER BY displayorder DESC, id ASC";
